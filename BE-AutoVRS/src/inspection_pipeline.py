...
from src.multiclass_detector import MultiClassOBBDetector
from src.singleclass_detector import SingleClassEnsemble
...
class InspectionPipeline:
    ...
    def __init__(self, config: PipelineConfig):
        ...
        self.multiclass_detector = MultiClassOBBDetector(
            model_path=config.multiclass_model_path,
            conf=config.conf_multiclass,
            imgsz=config.imgsz,
            device=config.device
        )
        
        self.single_ensemble = SingleClassEnsemble(
            engine_paths=config.single_engine_paths,
            engine_names=config.single_engine_names,
            conf=config.conf_single,
            imgsz=config.imgsz,
            device=config.device,
            iou_nms=config.iou_nms_single,
